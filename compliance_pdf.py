...
def build_pdf_bytes(entry, exporter_name, exporter_pin):
    ...
    if whisp_data:
        ...
        story.append(Paragraph(
            "This satellite assessment (source: FAO Open Foris Whisp) indicates detected "
            "tree cover loss status on the registered farm polygon since the EUDR reference "
            "date. This is one component of due diligence and does not by itself constitute "
            "full EUDR compliance, which also requires verification of legal land tenure, "
            "labor practices, and tax compliance.",
            body_s
        ))
    else:
        story.append(Paragraph(
